...
$(document).ready(function(){
	...
	$('.checkbox_websitee_1').parent().find('span.one_price').addClass('txt-light');
	$('input:checkbox[name=checkbox_websitee_1]:checked').each(function(){
		Totalwebsitee = Totalwebsitee + parseInt($(this).val());
		$(this).parent().find('span.one_price').removeClass('txt-light');
	});
	$('#websitee_price').html(Totalwebsitee);		
	
	$('.checkbox_websitee_1').on('click', function() {
		var currentvalue = $(this).val();
		...
		if(Totalwebsitee > 0){
			$("#websiteeSignup_1").removeClass('btn-disable');
			$('#websiteeSignup_1').unbind('click');
		}else{
			$("#websiteeSignup_1").addClass('btn-disable');
			$('#websiteeSignup_1').bind('click', false);
		}
	});
	...
	$('.checkbox_packaging_1').parent().find('span.one_price').addClass('txt-light');
	$('input:checkbox[name=checkbox_packaging_1]:checked').each(function(){
		Totalpackaging = Totalpackaging + parseInt($(this).val());
		$(this).parent().find('span.one_price').removeClass('txt-light');
	});
	$('#packaging_price').html(Totalpackaging);		
	
	$('.checkbox_packaging_1').on('click', function() {
		var currentvalue = $(this).val();
		...
		if(Totalpackaging > 0){
			$("#packagingSignup_1").removeClass('btn-disable');
			$('#packagingSignup_1').unbind('click');
		}else{
			$("#packagingSignup_1").addClass('btn-disable');
			$('#packagingSignup_1').bind('click', false);
		}
	});
